...
export default async function BoardPage({
  ...
  let query = supabase
    .from("board_posts")
    .select("id, category, title, source_name, pinned, created_at, created_by, guest_name, profiles(name), board_comments(count)")
  ...
  return (
    <div>
      ...
      {posts.length === 0 ? (
        <div className="card p-10 text-center text-brand-gray text-sm">No posts yet. Be the first to write one!</div>
      ) : (
          ...
          {posts.map((p) => (
            ...
            >
              ...
              <span className="text-xs text-brand-gray flex-shrink-0">
                {p.profiles?.name ?? p.guest_name ?? p.source_name ?? "Anonymous"} · {p.board_comments?.[0]?.count ?? 0} comments
              </span>
            </Link>
          ))}
        </div>
